...
/*SELECT DISTINCT CONCAT(m.surname,', ', m.firstname,' - ', fac.name)
FROM Bookings AS b
LEFT JOIN Members AS m
ON b.memid = m.memid
LEFT JOIN (SELECT facid, name
			FROM Facilities
			WHERE name LIKE '%tennis%court%') AS fac
...
/*SELECT f.name AS facility, CONCAT(m.surname,', ', m.firstname) AS name,
	CASE WHEN b.memid != 0 THEN b.slots*f.membercost
		...
		ELSE NULL END AS total_cost
FROM Bookings as b
LEFT JOIN (SELECT facid, name, membercost,guestcost FROM Facilities) as f
ON b.facid = f.facid
LEFT JOIN (SELECT memid, firstname, surname FROM Members) AS m
ON b.memid = m.memid
WHERE b.starttime BETWEEN '2012-09-14 00:00:00' AND '2012-09-14 23:59:59'
AND (CASE WHEN b.memid != 0 THEN b.slots*f.membercost
		WHEN b.memid = 0 THEN b.slots*f.guestcost
